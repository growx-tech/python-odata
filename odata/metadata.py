...
from odata.entity import EntityBase, declarative_base
...
class MetaData:
    ...
    namespaces = {"edm": "http://docs.oasis-open.org/odata/ns/edm", "edmx": "http://docs.oasis-open.org/odata/ns/edmx"}
    ...
    def _set_object_relationships(self, all_types):
        ...
        for entity in rich.progress.track(
            entities, "Entity relationships", transient=True, show_speed=True, update_period=1, disable=self.quiet
        ):  # type: EntityBase
                ...
                for _search_entity in entities:
                    ...
                    if type_ in (_search_type, _search_type_alias):
                        nav = NavigationProperty(
                            name, _search_entity, collection=is_collection, foreign_key=foreign_key
                        )
                        setattr(entity, name, nav)

    def _create_entities(self, all_types, entity_base_class, schemas, depth=1):  # noqa: C901
        orphan_entities = []
        with rich.progress.Progress(transient=True, disable=self.quiet) as progress:
            ...
            for schema in schemas:
                entity_task = progress.add_task(
                    f"Creating entities for {schema['name']}", total=len(schema.get("entities"))
                )
            ...
            if len(orphan_entities) > 0:
                if depth > 10:
                    errmsg = ("Types could not be resolved. " "Orphaned types: {}").format(", ".join(orphan_entities))
    ...
    def get_entity_sets(self, base=None):
        ...
        def enum_rewrite(name):
            return f"s{name}" if name.startswith("_") else name
        ...
        with rich.progress.Progress(transient=True, disable=self.quiet) as progress:
            ...
            for schema in schemas:
                ...
                for enum_type in schema["enum_types"]:
                    ...
                    names = [(enum_rewrite(i["name"]), i["value"]) for i in enum_type["members"]]
        ...
        for entity_set in rich.progress.track(
            entity_sets.values(), "Processing entity types ...", transient=True, disable=self.quiet
        ):
        ...
        self.log.info(f"Loaded {len(sets)} entity sets, total {len(all_types)} types")
        return base_class, sets, all_types

    def load_document(self):
        self.log.info(f"Loading metadata document: {self.url}")
    ...
    def _parse_enumtype(self, xmlq, enumtype_element, schema_name):
        ...
        enum = {"name": enum_name, "fully_qualified_name": ".".join([schema_name, enum_name]), "members": []}
    ...
    def parse_document(self, doc):  # noqa: C901
        schemas = []
        container_sets = {}
        actions = []
        functions = []
